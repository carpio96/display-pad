...
// capa de abstracción entre la libreria pantalla y LVGL
void my_disp_flush(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p)
{
    uint32_t ample = (area->x2 - area->x1 + 1);
    uint32_t alt = (area->y2 - area->y1 + 1);

    LCD_PintarImatge(area->x1, area->y1, ample, alt, &color_p->full);
